<template>
    <div>
        <div class="card">
            <Toolbar class="mb-4">
                <template #start>
                    <Button label="Nuevo Producto" icon="pi pi-plus" class="p-button-success mr-2"
                ...
                <template #end>
                    <FileUpload mode="Import" accept="image/*" :maxFileSize="1000000" label="Import"
            ...
            <DataTable ref="dt" :value="products" :lazy="true" v-model:selection="selectedProducts" dataKey="id"
                ...
                <template #header>
                    <div class="table-header flex flex-column md:flex-row md:justiify-content-between">
                        ...
                        <span class="p-input-icon-left">
                            ...
                            <InputText v-model="filters['global'].value" @keydown.enter="buscar()"
                                placeholder="Search..." />
                        </span>
                    </div>
                </template>
                ...
                <Column header="Image">
                    <template #body="slotProps">
                        ...
                        <img :src="`${asset}/${slotProps.data.imagen}`" :alt="slotProps.data.imagen"
                            class="product-image" />
                    </template>
                </Column>
            ...
            <Dialog v-model:visible="productDialog" :style="{width: '450px'}" :header="(product.id)?'Editar Producto':'Nuevo Producto'" :modal="true"
                class="p-fluid">
                ...
                <div class="field">
                    ...
                    <div class="formgrid grid">
                        <div class="field-radiobutton col-6" v-for="cat in categorias" :key="cat.id">
                            <RadioButton id="category1" name="category" :value="cat.id"
                ...
                <div class="formgrid grid">
                    <div class="field col">
                        ...
                        <InputNumber id="price" v-model="product.precio" mode="currency" currency="USD"
                            locale="en-US" />
                    </div>
                    <div class="field col">
            ...
            <Dialog header="Header" v-model:visible="displayModalImg" :breakpoints="{'960px': '75vw', '640px': '90vw'}"
                :style="{width: '50vw'}" :modal="true">
                <FileUpload name="demo[]" @upload="onUpload" :customUpload="true" @uploader="onImagenSeleccionada"
...
export default {
    ...
    setup() {
        ...
        const dt = ref();
        const confirmDeleteSelected=ref();
        ...
        const openDialogNuevo = () => {
            product.value = {} //se limpia la data para no muestra en el modal cuando se edita
            productDialog.value = true
        }
        const guardarProducto = async () => {
            // si hay un id de producto se actualiza
            if (product.value.id) {
                const { data } = await productoService.update(product.value, product.value.id)
                hideDialog()  //se cierra el modal
                getProductos()
        ...
        //boton cerrar modal al editar o guardar
        const hideDialog=()=>{
            productDialog.value=false;
